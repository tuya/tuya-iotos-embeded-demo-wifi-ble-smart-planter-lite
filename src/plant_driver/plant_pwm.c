...
{  
    ...
    pwm_init_config.ucList[3] = PWM_WARM_PORT;
    pwm_init_config.ucChannelNum = 4;
...
OPERATE_RET plant_pwm_set(USER_PWM_DUTY_T *pDutyData)
{
    ...
    percent = (FLOAT_T)(pDutyData->duty_green/1000.0);
    ...
    bk_pwm_update_param(pChannelList[2], pwm_period, percent * pwm_period);
    bk_pwm_start(pChannelList[2]);
    
    percent = (FLOAT_T)(pDutyData->duty_warm/1000.0);
    bk_pwm_update_param(pChannelList[3], pwm_period, percent * pwm_period);
    bk_pwm_start(pChannelList[3]);
...
OPERATE_RET plant_pwm_off(void)
{
    ...
    for(i = 0; i < pwm_init_config.ucChannelNum; i++) {
        bk_pwm_update_param(pChannelList[i], pwm_period, 0);
        bk_pwm_start(pChannelList[i]);
    }
